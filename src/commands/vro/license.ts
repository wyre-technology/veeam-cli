...
export function registerVroLicenseCommand(parent: Command, program: Command): void {
  const license = parent.command("license").description("License information");

  license
    .command("usage")
    .description("Get license usage for VRO server")
    .action(async () => {
      const format = program.opts().format as OutputFormat;
      try {
        const auth = resolveAuth("vro", program.opts().host);
        const response = await apiFetch("vro", auth, "/api/v7.21/LicenseUsage/Server");
